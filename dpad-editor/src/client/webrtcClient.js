...
const log = logging.createModuleLogger("webrtcClient");
...
const announceSignalingInfo = (room) => {
  signalingConns.forEach((conn) => {
    // only subcribe if connection is established, otherwise the conn automatically subscribes to all rooms
    if (conn.connected) {
      conn.send({ type: "subscribe", topics: [room.name], from: room.peerId });
...
const publishSignalingMessage = (conn, room, data) => {
  if (room.key) {
    ...
    conn.send({ type: "publish", topic: room.name, from: room.peerId, data });
  }
};
  ...
  constructor(url) {
    ...
    this.on("connect", () => {
      ...
      rooms.forEach((docId, room) => {
        this.send({ type: "subscribe", topics: [docId], from: room.peerId });
    ...
    this.on("message", (m) => {
      ...
      switch (m.type) {
        case "error": {
          ...
          if (room && m.to === room.peerId) {
            room.disconnect();
            emitter.emit("webrtc", { type: m.message });
          }
          break;
        }
        case "publish": {
